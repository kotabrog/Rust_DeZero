...
impl Function for Div {
    ...
    fn backward(&self, info: &FunctionInfo, variables: &mut VariableTable) -> Vec<usize> {
        ...
        let input1 = variables.get_variable_type(input_id1).expect("input1 is None");
        let input1 = match input1 {
            VariableType::F64(x) => x.data(),
        };
        let input_id2 = inputs[1];
        let input2 = variables.get_variable_type(input_id2).expect("input2 is None");
        let input2 = match input2 {
            VariableType::F64(x) => x.data(),
        };

        let gx1 = grad / input2;
        let gx2 = grad * &(- input1) / (input2.powi(2));
...
mod tests {
    ...
    fn div_forward() {
        ...
        let x = Variable::new(Tensor::new_from_num_vec(vec![4.0, 5.0, 6.0], vec![3]));
        let y = Variable::new(Tensor::new_from_num_vec(vec![1.0, 2.0, 3.0], vec![3]));
        ...
        let div = Div::new();
        let div_id = functions.add(FunctionWrapper::new(Box::new(div)));
        let div = functions.get_mut(div_id).expect("div is None");
        let outputs = div.call_mut(vec![x_id, y_id], &mut variables, false);
        let output = variables.get_variable_type(outputs[0]).expect("outputs[0] is None");
        let output = match output {
            VariableType::F64(x) => x.data(),
        };
        assert_eq!(*output, Tensor::new_from_num_vec(vec![4.0, 2.5, 2.0], vec![3]));
    }
    ...
    fn div_backward() {
        ...
        assert_eq!(*x_grad, Tensor::new_from_num_vec(vec![1.0 / 4.0, 1.0 / 5.0, 1.0 / 6.0], vec![3]));
        let y = variables.get_variable_type(y_id).expect("y is None");
        let y_grad = match y {
            VariableType::F64(y) => y.grad().unwrap(),
        };
        assert_eq!(*y_grad, Tensor::new_from_num_vec(vec![-1.0 / 16.0, -2.0 / 25.0, -3.0 / 36.0], vec![3]));
    }
}
