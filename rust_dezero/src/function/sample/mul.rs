...
mod tests {
    ...
    fn mul_forward() {
        ...
        let mul = Mul::new();
        let mul_id = functions.add(FunctionWrapper::new(Box::new(mul)));
        let mul = functions.get_mut(mul_id).expect("mul is None");
        let outputs = mul.call_mut(vec![x_id, y_id], &mut variables, false);
        let output = variables.get_variable_type(outputs[0]).expect("outputs[0] is None");
        let output = match output {
            VariableType::F64(x) => x.data(),
        };
        assert_eq!(*output, Tensor::new_from_num_vec(vec![4.0, 10.0, 18.0], vec![3]));
    }
    ...
    fn mul_backward() {
        ...
        assert_eq!(*x_grad, Tensor::new_from_num_vec(vec![4.0, 5.0, 6.0], vec![3]));
        let y = variables.get_variable_type(y_id).expect("y is None");
        let y_grad = match y {
            VariableType::F64(y) => y.grad().unwrap(),
        };
        assert_eq!(*y_grad, Tensor::new_from_num_vec(vec![1.0, 2.0, 3.0], vec![3]));
    }
}
